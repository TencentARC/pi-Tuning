...
def collate(samples, pad_idx, eos_idx):
    ...
    idxs = None
    if samples[0].get("idx", None) is not None:
        idxs = np.array([s['idx'] for s in samples])
...
class RTEDataset(OFADataset):
    ...
    def __getitem__(self, index):
        idx = None
        if 'test' not in self.dataset.file_path:
            ...
            sentence1, sentence2, label, idx = self.dataset[index]
            label = 'no'

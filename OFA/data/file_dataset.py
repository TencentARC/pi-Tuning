    ...
    def __init__(self, file_path, selected_col_ids=None, dtypes=None, separator="\t", cached_index=False):
        ...
        if selected_col_ids is None:
            ...
            self.selected_col_ids = list(
                range(len(open(self.file_path, encoding='utf-8').readline().rstrip("\n").split(self.separator))))
        else:
        ...
        if 'preft' not in file_path:
            self._init_seek_index()
            self._reader = self._get_reader()
            self.original = True
        else:
            self.data = pd.read_csv(file_path, sep='\t', header=None, error_bad_lines=False)

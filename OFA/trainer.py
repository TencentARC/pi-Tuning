    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            try:
                ...
                if getattr(self.cfg.model, "lora", False):
                    paths = getattr(self.cfg.model, "combine_lora_path", "")
                    if paths != "":
                        paths = paths.split(',')
                        if getattr(self.cfg.model, "lora_num", 1) > 1:
                            model_dict = self.model.state_dict()
                            pretrained_dict = {}
                            for i, path in enumerate(paths):
                                adapter_meta_dict = torch.load(path)['model']
                                for k in adapter_meta_dict:
                                    if 'lora_A' in k:
                                        pretrained_dict[k.replace('lora_A', str(i)+'.lora_A')] = adapter_meta_dict[k]
                                    elif 'lora_B' in k:
                                        pretrained_dict[k.replace('lora_B', str(i)+'.lora_B')] = adapter_meta_dict[k]
                                    elif 'proj' in k and 'image' not in k and 'projection' not in k:
                                        pretrained_dict[k.replace('proj', 'proj.' + str(i))] = adapter_meta_dict[k]

                                model_dict.update(pretrained_dict)
                            self.model.load_state_dict(model_dict)
                        else:
                            model_dict = self.model.state_dict()
                            adapter_meta_dict = torch.load(paths[0])['model']
                            pretrained_dict = {k: adapter_meta_dict[k] for k in
                                               adapter_meta_dict if 'lora' in k}
                            model_dict.update(pretrained_dict)
                            self.model.load_state_dict(model_dict)

                if getattr(self.cfg.model, "adapter", False):
                    paths = getattr(self.cfg.model, "combine_adapter_path", "")
                    if paths != "":
                        paths = paths.split(',')
                        if getattr(self.cfg.model, "adapter_num", 1) > 1:
                            model_dict = self.model.state_dict()
                            for i, path in enumerate(paths):
                                adapter_meta_dict = torch.load(path)['model']
                                pretrained_dict = {k.replace('adapter1', 'adapters1.' + str(i)): adapter_meta_dict[k]
                                                   for k in adapter_meta_dict if 'adapter1' in k}
                                pretrained_dict.update(
                                    {k.replace('adapter2', 'adapters2.' + str(i)): adapter_meta_dict[k] for k in
                                     adapter_meta_dict if 'adapter2' in k})
                                model_dict.update(pretrained_dict)
                            self.model.load_state_dict(model_dict)
                        else:
                            model_dict = self.model.state_dict()
                ...
                if getattr(self.cfg.model, "encoder_prompt", False):
                    ...
                    if paths != "":
                        ...
                        if len(paths) > 1:
                            ...
                            for i, path in enumerate(paths):
                                adapter_meta_dict = torch.load(path)['model']
                                pretrained_dict = {k.replace('encoder_prompt_encoder', 'encoder_prompt_encoder.' + str(i)): adapter_meta_dict[k]
                                                   for k in adapter_meta_dict if 'encoder_prompt_encoder' in k}
                                pretrained_dict.update(
                                    {k.replace('decoder_prompt_encoder', 'decoder_prompt_encoder.' + str(i)): adapter_meta_dict[k] for k in
                                     adapter_meta_dict if 'decoder_prompt_encoder' in k})
                                model_dict.update(pretrained_dict)
                            self.model.load_state_dict(model_dict)
                        else:
                            model_dict = self.model.state_dict()
                            adapter_meta_dict = torch.load(paths[0])['model']
                            pretrained_dict = {k: adapter_meta_dict[k] for k in
                                               adapter_meta_dict if 'prompt' in k}
                            model_dict.update(pretrained_dict)
                            self.model.load_state_dict(model_dict)
            ...
            try:
                with maybe_no_sync():
                    ...
                    loss, sample_size_i, logging_output = self.task.train_step(
                        ...
                        task_emb=self.cfg.common.task_emb,
                        **extra_kwargs,
                    )
                    del loss
                    if self.cfg.common.task_emb:
                        if len(self.FIM_outputs) == 0:
                            for key in logging_output['FIM']:
                                self.FIM_outputs[key] = logging_output['FIM'][key]
                        else:
                            for key in logging_output['FIM']:
                                self.FIM_outputs[key] += logging_output['FIM'][key]
                        logging_output.pop('FIM')

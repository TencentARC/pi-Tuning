...
def main(cfg: DictConfig, **kwargs):
    ...
    TP_sum = torch.FloatTensor([0]).cuda()
    FP_sum = torch.FloatTensor([0]).cuda()
    FN_sum = torch.FloatTensor([0]).cuda()
    TP = FP = FN = None
    for sample in progress:
        ...
        with torch.no_grad():
            if kwargs["zero_shot"] and task.cfg._name == 'vqa_gen':
        ...
        TP_sum += sum(TP) if TP is not None else 0
        FP_sum += sum(FP) if FP is not None else 0
        FN_sum += sum(FN) if FN is not None else 0

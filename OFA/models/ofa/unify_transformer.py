...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--adapter-interpolation', default=False, action='store_true',
                            help='use attention to do adapter interpolation')
        parser.add_argument('--lora', default=False, action='store_true',
                            help='use lora to tune')
        parser.add_argument('--lora-r', type=int, metavar='N',
                            help='lora rank')
        parser.add_argument('--linear-connect', default=False, action='store_true',
                            help='linear mode connectivity analysis')
        parser.add_argument('--model-soup', default=False, action='store_true',
                            help='use model soup')
        parser.add_argument('--visualize', default=False, action='store_true',
                            help='loss landscape visualize')
        parser.add_argument('--ckpt2', type=str, default='',
                            help='checkpoint2 path for loss landscape visualization')
        parser.add_argument('--ckpt3', type=str, default='',
                            help='checkpoint3 path for loss landscape visualization')
        parser.add_argument('--adapter', action='store_true',
                            help='use adapter in the model')
        parser.add_argument('--lora-num', type=int, metavar='N',
                            help='num of used lora')
        parser.add_argument('--adapter-num', type=int, metavar='N',
                            help='num of used adapter')
        parser.add_argument('--prompt-num', type=int, metavar='N',
                            help='num of used prompt')
        parser.add_argument('--adapter-dim', type=int, metavar='N',
                            help='adapter-down-dim')
        parser.add_argument('--combine-lora-path', type=str, default='',
                            help='paths of loras to be combined')
        parser.add_argument('--combine-prompt-path', type=str, default='',
                            help='paths of prompts to be combined')
        parser.add_argument('--combine-adapter-path', type=str, default='',
                            help='paths of adapters to be combined')
        parser.add_argument('--learnable-scale', default=False, action='store_true',
                            help='make adapter scale learnable')
        parser.add_argument('--init-scale', type=str, default='',
                            help='init value of adapter scale')
    ...
    def build_model(cls, args, task):
        ...
        if getattr(args, "adapter", False) or getattr(args, "use_adapter", False) or getattr(args, "encoder_prompt",
                                                                                             False) or getattr(
            ...
            if getattr(args, "prompt_num", None) and getattr(args, "prompt_num", None) != 1:
                encoder.prompt_weight_scale.requires_grad_(True)
                decoder.prompt_weight_scale.requires_grad_(True)
                if not getattr(args, "learnable_sclae", False):
                    encoder.encoder_prompt_encoder.requires_grad_(False)
                    decoder.decoder_prompt_encoder.requires_grad_(False)
                else:
                    encoder.encoder_prompt_encoder.requires_grad_(True)
                    decoder.decoder_prompt_encoder.requires_grad_(True)
            if getattr(args, "adapter", False):
                if getattr(args, "adapter_num", None) is None or getattr(args, "adapter_num", None) == 1:
                    for idx, layer in enumerate(encoder.layers):
                        layer.adapter1.requires_grad_(True)
                        layer.adapter2.requires_grad_(True)
                    for idx, layer in enumerate(decoder.layers):
                        layer.adapter1.requires_grad_(True)
                        layer.adapter2.requires_grad_(True)
                else:
                    for idx, layer in enumerate(encoder.layers):
                        layer.adapters1.requires_grad_(True)
                        layer.adapters2.requires_grad_(True)
                        if getattr(args, "learnable_scale", False):
                            layer.adapter_interpolation_weight1.requires_grad_(True)
                            layer.adapter_interpolation_weight2.requires_grad_(True)
                    for idx, layer in enumerate(decoder.layers):
                        layer.adapters1.requires_grad_(True)
                        layer.adapters2.requires_grad_(True)
                        if getattr(args, "learnable_scale", False):
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.prompt_num = getattr(args, "prompt_num", None)
        if getattr(args, "encoder_prompt", False):
            if not self.prompt_num or self.prompt_num == 1:
                ...
                self.prompt_weight_scale = nn.Parameter(torch.ones((args.encoder_layers, self.prompt_num)),
                                                        requires_grad=True)
                self.encoder_prompt_encoder = nn.ModuleList([PromptEncoder(
                    type=args.encoder_prompt_type,
                    length=args.encoder_prompt_length,
                    projection=args.encoder_prompt_projection,
                    embed_dim=args.encoder_embed_dim,
                    proj_dim=args.encoder_prompt_dim,
                    layers=args.encoder_layers,
                    vocab_size=args.vocab_size) for _ in range(self.prompt_num)])
    ...
    def build_encoder_layer(self, args, drop_path_rate=0.0):
        init_scale = None
        if getattr(args, 'init_scale', "") != "":
            init_scale = torch.tensor([float(weight) for weight in args.init_scale.split(',')])
        layer = TransformerEncoderLayer(args, drop_path_rate=drop_path_rate,
                                        ...
                                        adapter_num=getattr(args, "adapter_num", None),
                                        use_lora=getattr(args, "lora", False),
                                        lora_r=getattr(args, "lora_r", 16),
                                        init_scale=init_scale,
                                        lora_num=getattr(args, "lora_num", None))
    ...
    def get_encoder_prompt(self, prompt_tokens, prompt_idx=None):
        if prompt_idx is None:
            past_key_values = self.encoder_prompt_encoder(prompt_tokens)
        else:
            past_key_values = self.encoder_prompt_encoder[prompt_idx](prompt_tokens)
            ...
            if not self.prompt_num or self.prompt_num == 1:
                ...
                for i in range(self.prompt_num):
                    if self.args.encoder_prompt_type in ("prefix"):
                        ...
                        prompt_tokens = prompt_tokens.unsqueeze(0).expand(bsz, -1)
                        prompt_padding_mask = torch.zeros_like(prompt_tokens).to(prompt_tokens.device)
                    prompt_kv_list.append(self.get_encoder_prompt(prompt_tokens, prompt_idx=i))
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if self.args.encoder_prompt:
                if not self.prompt_num or self.prompt_num == 1:
                    ...
                    for i in range(self.prompt_num):
                        if self.args.encoder_prompt_type != "prompt":
                            prompt_kv = prompt_kv_list[i][idx]
                        else:
                            if idx == 0:
                                prompt_kv = prompt_kv_list[i][idx]
                            else:
                                prompt_kv = None
                        prompt_kvs.append(prompt_kv.unsqueeze(-1))
                    prompt_kvs = torch.cat(prompt_kvs, dim=-1)
                    prompt_kv = torch.sum(prompt_kvs * F.softmax(self.prompt_weight_scale[idx, :]), dim=-1)
            else:
                prompt_kv = None
            x = layer(
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        if getattr(args, "decoder_prompt", False):
            if not self.prompt_num or self.prompt_num == 1:
                ...
                self.prompt_weight_scale = nn.Parameter(torch.ones((args.encoder_layers, self.prompt_num)),
                                                        requires_grad=True)
                self.decoder_prompt_encoder = nn.ModuleList([PromptEncoder(
                    type=args.decoder_prompt_type,
                    length=args.decoder_prompt_length,
                    projection=args.decoder_prompt_projection,
                    embed_dim=args.decoder_embed_dim,
    ...
    def get_decoder_prompt(self, prompt_tokens, prompt_idx=None):
        if prompt_idx is None:
            past_key_values = self.decoder_prompt_encoder(prompt_tokens)
        else:
            past_key_values = self.decoder_prompt_encoder[prompt_idx](prompt_tokens)
            ...
            if not self.prompt_num or self.prompt_num == 1:
                ...
                for i in range(self.prompt_num):
                    if self.args.decoder_prompt_type in ("prefix"):
                        ...
                        prompt_tokens = prompt_tokens.unsqueeze(0).expand(bsz, -1)
                        prompt_padding_mask = torch.zeros_like(prompt_tokens).to(prompt_tokens.device)
                    prompt_kv_list.append(self.get_decoder_prompt(prompt_tokens, prompt_idx=i))

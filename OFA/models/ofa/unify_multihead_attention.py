...
class MultiheadAttention(nn.Module):
    ...
    def __init__(
        ...
        self.lora_num = lora_num if lora_num != 1 else None
        ...
        if not use_lora:
            ...
            if self.lora_num is None:
                self.k_proj = quant_noise(
                    lora.Linear(self.kdim, embed_dim, bias=bias, r=lora_r), q_noise, qn_block_size
                )
                self.v_proj = quant_noise(
                    lora.Linear(self.vdim, embed_dim, bias=bias, r=lora_r), q_noise, qn_block_size
                )
                self.q_proj = quant_noise(
                    lora.Linear(embed_dim, embed_dim, bias=bias, r=lora_r), q_noise, qn_block_size
                )
                ...
                self.lora_interpolation_weight = nn.Parameter(torch.ones(lora_num), requires_grad=True)

                self.k_proj = nn.ModuleList([quant_noise(
                    lora.Linear(self.kdim, embed_dim, bias=bias, r=lora_r), q_noise, qn_block_size
                ) for _ in range(self.lora_num)])
                self.v_proj = nn.ModuleList([quant_noise(
                    lora.Linear(self.vdim, embed_dim, bias=bias, r=lora_r), q_noise, qn_block_size
                ) for _ in range(self.lora_num)])
                self.q_proj = nn.ModuleList([quant_noise(
                    lora.Linear(embed_dim, embed_dim, bias=bias, r=lora_r), q_noise, qn_block_size
                ) for _ in range(self.lora_num)])

                self.out_proj = nn.ModuleList([quant_noise(
                    lora.Linear(embed_dim, embed_dim, bias=bias, r=lora_r), q_noise, qn_block_size
                ) for _ in range(self.lora_num)])
    ...
    def q_projection(self, x):
        if not self.use_lora or self.lora_num is None:
            return self.q_proj(x)
        else:
            z = torch.cat([proj(x).unsqueeze(dim=-1) for i, proj in enumerate(self.q_proj)], dim=-1)
            return torch.sum(z*F.softmax(self.lora_interpolation_weight), dim=-1)

    def k_projection(self, x):
        if not self.use_lora or self.lora_num is None:
            return self.k_proj(x)
        else:
            z = torch.cat([proj(x).unsqueeze(dim=-1) for i, proj in enumerate(self.k_proj)], dim=-1)
            return torch.sum(z*F.softmax(self.lora_interpolation_weight), dim=-1)

    def v_projection(self, x):
        if not self.use_lora or self.lora_num is None:
            return self.v_proj(x)
        else:
            z = torch.cat([proj(x).unsqueeze(dim=-1) for i, proj in enumerate(self.v_proj)], dim=-1)
            return torch.sum(z*F.softmax(self.lora_interpolation_weight), dim=-1)

    def o_projection(self, x):
        if not self.use_lora or self.lora_num is None:
            return self.out_proj(x)
        else:
            z = torch.cat([proj(x).unsqueeze(dim=-1) for i, proj in enumerate(self.out_proj)], dim=-1)
            return torch.sum(z*F.softmax(self.lora_interpolation_weight), dim=-1)
        ...
        if self.lora_num is None:
            ...
            for i in range(self.lora_num):
                if self.qkv_same_dim:
                    ...
                    nn.init.xavier_uniform_(self.k_proj[i].weight)
                    nn.init.xavier_uniform_(self.v_proj[i].weight)
                    nn.init.xavier_uniform_(self.q_proj[i].weight)
                nn.init.xavier_uniform_(self.out_proj[i].weight)

                if self.out_proj[i].bias is not None:
                    nn.init.constant_(self.out_proj[i].bias, 0.0)
    ...
    def forward(
        ...
        if self.self_attention and self_attn_mask is None:
            ...
            k = self.k_projection(query)
            v = self.v_projection(query)

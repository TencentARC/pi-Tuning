...
def load_model_ensemble_and_task(
    ...
    for filename in filenames:
        ...
        for shard_idx in range(num_shards):
            ...
            if getattr(cfg.model, "visualize", False):
                ckpt2_path = getattr(cfg.model, "ckpt2")
                ckpt3_path = getattr(cfg.model, "ckpt3")
                ckpt1 = model.state_dict()
                ckpt2 = torch.load(ckpt2_path)['model']
                ckpt3 = torch.load(ckpt3_path)['model']
                ...
                for k in ckpt1:
                    if 'adapter' in k:
                        u[k] = ckpt2[k] - ckpt1[k]
                        w2_w1_2 += torch.norm(u[k])**2
                        w3_w1_x_w2_w1 += torch.sum((ckpt3[k]-ckpt1[k]).mul(ckpt2[k]-ckpt1[k]))
                ...
                for k in ckpt1:
                    if 'adapter' in k:
                        v[k] = ckpt3[k]-ckpt1[k] - w3_w1_x_w2_w1/w2_w1_2 * (ckpt2[k]-ckpt1[k])
                        v_2 += torch.norm(v[k])**2
                for k in ckpt1:
                    if 'adapter' in k:
                        u_base[k] = u[k] / torch.sqrt(u_2)
                        v_base[k] = v[k] / torch.sqrt(v_2)
                print(f'norm of u is {torch.sqrt(u_2)}')
                print(f'norm of v is {torch.sqrt(v_2)}')
                print(w3_w1_x_w2_w1/torch.sqrt(w2_w1_2))
                model_dict = model.state_dict()
                for k in model_dict:
                    if 'adapter' in k:
                        model_dict[k] = ckpt1[k] + visualize_scale[0]*u_base[k] + visualize_scale[1]*v_base[k]
                model.load_state_dict(model_dict)

            if getattr(cfg.model, "model_soup", None):
                paths = getattr(cfg.model, "combine_adapter_path", "")
                paths = paths.split(',')
                model_dict = model.state_dict()
                for path in paths:
                    adapter_meta_dict = torch.load(path)['model']
                    pretrained_dict = {k: adapter_meta_dict[k]
                                        for k in adapter_meta_dict if 'adapter' in k}

                    for k in pretrained_dict:
                        model_dict[k] = pretrained_dict[k] + model_dict[k]
                
                for k in model_dict:
                    if 'adapter' in k:
                        model_dict[k] = model_dict[k] / (len(paths) + 1)
                model.load_state_dict(model_dict)
                
            if getattr(cfg.model, "linear_connect", None):
                if getattr(cfg.model, "adapter", False):
                    paths = getattr(cfg.model, "combine_adapter_path", "")
                    paths = paths.split(',')
                    model_dict = model.state_dict()
                    ...
                    for k in pretrained_dict:
                        pretrained_dict[k] = alpha*pretrained_dict[k] + (1-alpha)*model_dict[k]
                    model_dict.update(pretrained_dict)
                    model.load_state_dict(model_dict)
                elif getattr(cfg.model, "prompt", False):
                    paths = getattr(cfg.model, "combine_prompt_path", "")
                    paths = paths.split(',')
                    model_dict = model.state_dict()
                    adapter_meta_dict = torch.load(paths[0])['model']
                    pretrained_dict = {k: adapter_meta_dict[k]
                                       for k in adapter_meta_dict if 'prompt' in k}
                    for k in pretrained_dict:
                        pretrained_dict[k] = alpha * pretrained_dict[k] + (1 - alpha) * model_dict[k]
                    model_dict.update(pretrained_dict)
                    model.load_state_dict(model_dict)
                elif getattr(cfg.model, "bitfit", False):
                    paths = getattr(cfg.model, "combine_adapter_path", "")
                    paths = paths.split(',')
                    model_dict = model.state_dict()
                    adapter_meta_dict = torch.load(paths[0])['model']
                    ...
                    pretrained_dict = {k: adapter_meta_dict[k]
                                       for k in adapter_meta_dict if ("layer_norm" in k and "bias" in k) or ("fc" in k and "bias" in k)}

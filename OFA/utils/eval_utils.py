...
def eval_glue(task, generator, models, sample, **kwargs):
    hyps = get_hyps(task, generator, models, sample, **kwargs)
    results = []
    label_map = task_label_map[task.cfg._name]
    if sample["idxs"] is not None:
        for idx, hyp in zip(sample["idxs"].tolist(), hyps):
            results.append([idx, label_map[hyp]])
        return results, None
    else:
        scores = [ref_dict.get(hyp, 0) for ref_dict, hyp in zip(sample['ref_dict'], hyps)]
        return [], scores
...
def get_hyps(task, generator, models, sample, **kwargs):
...
def eval_step(task, generator, models, sample, **kwargs):
    ...
    elif task.cfg._name == 'image_classify' or task.cfg._name == 'unified_img_cls':
...
def merge_results(task, cfg, logger, score_cnt, score_sum, results, TP_sum=None, FP_sum=None, FN_sum=None):
    if task.cfg._name == 'image_gen':
        ...
        if cfg.distributed_training.distributed_world_size > 1:
            ...
            if TP_sum is not None:
                dist.all_reduce(TP_sum.data)
                dist.all_reduce(FP_sum.data)
                dist.all_reduce(FN_sum.data)

...
class OFAConfig(FairseqDataclass):
    ...
    extra_task: Optional[str] = field(
        default=None,
        metadata={"help": "extra_task"},
    )
    extra_data: Optional[str] = field(
        default=None,
        metadata={"help": "extra_data"},
    )
    extra_selected_cols: Optional[str] = field(
        default=None,
        metadata={"help": "extra_selected_cols"},
    )
...
class OFATask(FairseqTask):
    ...
    def train_step(
        self, sample, model, criterion, optimizer, update_num, ignore_grad=False, task_emb=False, **extra_kwargs
    ):
        """
        ...
        Args:
            ...
            task_emb(bool): calculate task embedding and no update parameter
        ...
        if ignore_grad:
            loss *= 0
        if not task_emb:
            # if sample['dataset_type'] == 'image_classify':
            ...
            for name, parameter in model.named_parameters():
                if parameter.grad is not None:
                    # x = parameter.grad.reshape(-1)
                    # mean = x.mean(dim=0)
                    # std = x.std(dim=0, unbiased=False)
                    FIM[name] = parameter.grad ** 2
            logging_output['FIM'] = FIM
